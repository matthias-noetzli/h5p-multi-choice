 ...
 * @returns {H5P.MultiChoiceMedia}
 * @constructor
 */
H5P.MultiChoiceMedia = function(options, contentId, contentData) {
    if (!(this instanceof H5P.MultiChoiceMedia))
        return new H5P.MultiChoiceMedia(options, contentId, contentData);
    var self = this;
    this.contentId = contentId;
    this.contentData = contentData;
    H5P.Question.call(self, 'multichoice-media');
    var $ = H5P.jQuery;
    ...
    /**
     * Add tip to element
     * @param {jQuery} $element Element that tip will be added to
     * @param {string} feedback Tip string
     */
    var addTip = function($element, tip) {
        $feedbackDialog = $('' +
            '<div class="h5p-feedback-dialog">' +
            '<div class="h5p-feedback-inner">' +
            '<div class="h5p-feedback-text">' + tip + '</div>' +
            '</div>' +
            '</div>');
    ...
    var addFeedback = function($element, feedback) {
        $feedbackDialog = $('' +
            '<div class="h5p-feedback-dialog">' +
            '<div class="h5p-feedback-inner">' +
            '<div class="h5p-feedback-text"></div>' +
            '</div>' +
            '</div>');

        $container = $(".h5p-feedback-text", $feedbackDialog);

        feedback.forEach(content => addLibrary($container, content));
    ...
    var addFeedbackToggle = function($element, feedback) {

        var $alternativeContainer = $element.find($('.h5p-alternative-container'));

        // Add feedback icon
        var $wrap = $('<div/>', {
            'class': 'h5p-multichoice-media-feedbackicon-wrap',
            'aria-label': params.UI.feedbackAvailable + '.'
        });

        var $multichoiceFeedbackIcon = $('<div>', {
            'role': 'button',
            'tabindex': 0,
            'title': params.UI.feedbackLabel,
            'aria-label': params.UI.feedbackLabel,
            'aria-expanded': false,
            'class': 'multichoice-feedback-icon',
            appendTo: $wrap
        });

        var feedbackIconHtml = '<span class="joubel-icon-comment-normal">' +
            '<span class="h5p-icon-shadow"></span>' +
            '<span class="h5p-icon-speech-bubble"></span>' +
            '<span class="h5p-icon-question"></span>' +
            '</span>';

        $multichoiceFeedbackIcon.append(feedbackIconHtml);

        $multichoiceFeedbackIcon.click(function() {
            var $feedbackContainer = $multichoiceFeedbackIcon.parents('.h5p-answer');
            var openFeedback = !$feedbackContainer.children('.h5p-feedback-dialog').is($feedbackDialog);
            removeFeedbackDialog();

            // Do not open feedback if it was open
            if (openFeedback) {
                $multichoiceFeedbackIcon.attr('aria-expanded', true);

                //Add feedback dialog
                addFeedback($feedbackContainer, feedback);
                $feedbackDialog.addClass('h5p-has-feedback');
            } else {
                $multichoiceFeedbackIcon.attr('aria-expanded', false);
            }
        ...
        $alternativeContainer.append($wrap);
    }
    ...
    self.registerDomElements = function() {
        ...
        const $introductionElement = $('<div>', {
            id: params.label
        })
        options.question.forEach(content => {
            addLibrary($introductionElement, content);
        });
        // Register Introduction
        self.setIntroduction($introductionElement);
        ...
        var $answers = $('.h5p-answer', $myDom).each(function(i) {
            ...
            // Add tip
            var $wrap = $('<div/>', {
                'class': 'h5p-multichoice-media-tipwrap',
        ...
        self.addButton('try-again', params.UI.tryAgainButton, function() {
            ...
            if (params.behaviour.randomAnswers) {
                ...
                for (i = 0; i < answersDisplayed.length; i++) {
                    tip[i] = $(answersDisplayed[i]).find('.h5p-multichoice-media-tipwrap');
                }
    ...
    H5P.MultiChoiceMedia.counter = (H5P.MultiChoiceMedia.counter === undefined ? 0 : H5P.MultiChoiceMedia.counter + 1);
    params.role = (params.behaviour.singleAnswer ? 'radiogroup' : 'group');
    params.label = 'h5p-mcq' + H5P.MultiChoiceMedia.counter;
    ...
    /**
     * Add library to element
     * @param {jQuery} $element Element that content will be added to
     * @param {Object} libraryData Library data
     */
    var addLibrary = function($element, libraryData) {
        // Initialize content
        var $wrapper = $('<div>');
        $wrapper.addClass('h5p-library-content');
        $element.append($wrapper);

        const runnable = H5P.newRunnable(libraryData, self.contentId, $wrapper, false);
        // Trigger resize events on the runnable:
        self.on('resize', function(event) {
            runnable.trigger('resize', event);
        });
...
H5P.MultiChoiceMedia.prototype = Object.create(H5P.Question.prototype);
H5P.MultiChoiceMedia.prototype.constructor = H5P.MultiChoiceMedia;